...
para <- function(halfsibs, cpus = 1, option = "bmh", type = "SOCK", bmh_forwardVectorSize = 30, 
		bmh_excludeFP = TRUE, bmh_nsap = 3, pmMethod = "constant")
{
	...
	rec <- function(x)
	{
		if (!is.null(x))
		{
			y <- pm(bmh(x, forwardVectorSize = bmh_forwardVectorSize, excludeFP = bmh_excludeFP, 
							nsap = bmh_nsap), method = pmMethod)
			y <- apply(y, 2, sum)
			y[y>nrow(x)/2]=0
	...
	if (cpus == 1)
	{
		if (option == "bmh")
		{
			allhs <- lapply(halfsibs, bmhRnull)
		}
		if (option == "aio")
		{
			allhs <- lapply(halfsibs, aioRnull)
		}
		if (option == "rec")
		{
			allhs <- lapply(halfsibs, rec)
		}
		if (option == "pm")
		{
			allhs <- lapply(halfsibs, function(x)pm(bmh(x),pmMethod))
		}
		if (option == "ssp")
		{
			allhs <- lapply(halfsibs, function(x)
					{
						ssp(bmh(x, forwardVectorSize = bmh_forwardVectorSize, excludeFP = bmh_excludeFP, 
										nsap = bmh_nsap), x)
					})
		}
	}
