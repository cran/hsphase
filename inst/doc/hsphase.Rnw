...
In case the pedigree is not available or the pedigree is not reliable, the
...
\emph{Note 1:} The SNP and the animals' IDs should not contain a double quotes.\\
...
A flat file with genotypes (ID x SNP), where the SNP names are in the first row
...
The \emph{hss} function generates a list of matrices (ID x SNP), one for each
of the half-sib groups that have at least 4 offspring per sire. The input are
the genotype and pedigree files. The names of the list are the names of the sires in the pedigree file.
...
file. The output is also a list of matrices (ID x SNP, one for each family and
...
built manually; it's simply a split list of numeric matrices (ID x SNP with 0,
...
analysis and can be any of the above described \emph{bmh}, \emph{ssp} or
\emph{aio} and also \emph{rec}. The \emph{rec} option returns a list with the
sum of recombinations between SNP for each chromosome in each half-sib group.
...
\subsection{\emph{Blocking} Structure Plot (imageplot)}
...
The heatmap assists indentification of pedigree errors and can help to check if the pedigree reconstruction results seem correct. The
...
function to create, respectively, the \emph{RowSideColors} and \emph{ColSideColors}. In practice either can be freely interchanged to colour code the side barss of the heatmap.
...
The \emph{pm} function utilizes the block structure matrix (from \emph{bmh}) to
...
This fuction can be useful to identify mapping errors. For example, if (very) large recombination rates are observed at a particular SNP across families.
...
\emph{Note: } This function utilizes OpenMP in Linux and the number of
\emph{cpus} is only valid in Linux.


\subsection{Pedigree Reconstruction of half-sib families (rpoh)}
\label{sec: Reconstruction pedigree of half-sib (rpoh)}
The \emph{roph} function reconstructs half-sib families; i.e. splits the individuals into half-sib groups. 

\begin{verbatim}
rpoh(genotypeMatrix, ohg(genotype), method = "simple")
\end{verbatim}

Two methods \emph{simple} and \emph{recombinations} can be utilised to
reconstruct the pedigree. The \emph{simple} methgod calculate the maximum number
of opposing homozygote in the half-sib family based on the provided
\emph{Intercept} and \emph{coefficient} (The defualt values are tested with
cattle and sheep genotypes). The \emph{recombinations}  method caculte the numer
of recombinations utising the \emph{bmh} and \emph{rec} functions.\\ 


\emph{Note 1:} The functions \emph{ohg} and \emph{roph} with
...
\emph{Note 2:} The first argument of \emph{roph} function (genotypeMatrix) must
use only genotypic data from a single chromosome.
...
The \emph{pedigreeNaming} function tries to link the inferred half-sib family groups to the sire IDs in the original pedigree and fix errors. This works well if the original pedigree is relatively correct
 but individuals will be missasigned if most of the individuals originally allocated to a sire are not its offspring.

\section{Imputation}
The \emph{impute} function impute the low density SNP marker to high density
marker utlising the high density haplotype of sire.

\begin{verbatim}
impute(halfsib_parental_ld, sire_ld, sire_hd)
\end{verbatim}

The \emph{halfsib\_parental\_ld} is the parental strand of half--sibs with
low density markers (output of \emph{phf} - Section \ref{sec:
Half-Sib Family Phasing (phf)}). The \emph{sire\_ld} is the haplotype of sire
with low density marker (output of \emph{ssp} - Section \ref{sec: Sire
Imputation and Phasing (ssp)}). The \emph{sire\_hd} is the haplotype of sire
...
recombinationBlocksList <- para(halfsib, cpus=20, option="bmh", type = "SOCK")  
