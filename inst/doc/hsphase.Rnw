...
\begin{verbatim}
rpoh(oh = ohg(genotype), snpNooh = 50, method = "simple")
\end{verbatim}

Two methods \emph{simple} and \emph{recombinations} can be utilised to
reconstruct the pedigree. The \emph{simple} method calculate the maximum number
of opposing homozygote in the half-sib family based on the provided
\emph{intercept} and \emph{coefficient} (The default values are tested with
cattle and sheep genotypes). The \emph{recombinations}  method calculate the number
of recombinations utilising the \emph{bmh} and \emph{rec} functions.\\ 
...
\emph{Note 2:} The first argument of \emph{rpoh} function (genotypeMatrix) must
use only genotypic data from a single chromosome.\\

\emph{Note 3:} The \emph{snpNooh} is the number of SNPs (divided by 1000) used to create opposing homozygote matrix.
...
\begin{verbatim}
impute(halfsib_paternal_ld, sire_ld, sire_hd)
\end{verbatim}

The \emph{halfsib\_paternal\_ld} is the paternal strand of half--sibs with
...
\emph{Note:} The \emph{halfsib\_paternal\_ld} and \emph{sire\_hd} must have \emph{colnames} which are the SNP names.\\
